...
import { FRAME_GENERATION_PRESENTATION_QUEUE_CAPACITY } from "./frameGenerationCapacity";

const DEFAULT_SOURCE_FRAME_MS = 1000 / 60;
const MIN_SOURCE_FRAME_MS = 1000 / 240;
const MAX_SOURCE_FRAME_MS = 100;
const MIN_DISPLAY_FRAME_MS = 1000 / 300;
const MAX_DISPLAY_FRAME_MS = 100;
const DISPLAY_CADENCE_SMOOTHING = 0.25;
const MAX_DISPLAY_TO_SOURCE_RATIO_FOR_INTERPOLATION = 0.65;
...
export class FrameGenerationPipeline {
  ...
  private lastPresentationRafMs: number | null = null;
  private displayFrameDurationMs: number | null = null;
  ...
  private stopRuntime(): void {
    ...
    this.lastPresentationRafMs = null;
    this.displayFrameDurationMs = null;
  ...
  private captureFrame(now: number, mediaTimeSeconds: number | null): void {
    ...
    if (this.passthroughMode) {
      this.enqueuePresentation(now, captured.retain());
      previous.release();
      this.currentFrame = captured;
  ...
  private readonly presentFrame = (now: DOMHighResTimeStamp): void => {
    ...
    const backend = this.backend;
    this.observeDisplayCadence(now);
  ...
  private observeDisplayCadence(now: number): void {
    const previousRafMs = this.lastPresentationRafMs;
    this.lastPresentationRafMs = now;
    if (previousRafMs === null || this.passthroughMode) return;

    const intervalMs = now - previousRafMs;
    if (
      !Number.isFinite(intervalMs)
      || intervalMs < MIN_DISPLAY_FRAME_MS
      || intervalMs > MAX_DISPLAY_FRAME_MS
    ) {
      return;
    }

    this.displayFrameDurationMs = this.displayFrameDurationMs === null
      ? intervalMs
      : this.displayFrameDurationMs * (1 - DISPLAY_CADENCE_SMOOTHING)
        + intervalMs * DISPLAY_CADENCE_SMOOTHING;
    if (
      this.displayFrameDurationMs
      <= this.sourceFrameDurationMs * MAX_DISPLAY_TO_SOURCE_RATIO_FOR_INTERPOLATION
    ) {
      return;
    }
    ...
    this.clearPresentationQueue();
    if (this.currentFrame) {
      this.enqueuePresentation(now, this.currentFrame.retain());
    }
  }
