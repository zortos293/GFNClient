...
//!
//! Handles the WebSocket-based signaling for WebRTC connection setup.
//! Protocol: wss://{server}/nvst/sign_in?peer_id=peer-{random}&version=2
...
impl GfnSignaling {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        // Use client_async (NOT client_async_tls) since we already have a TLS stream
                                ...
                                if let Ok(inner) = serde_json::from_str::<Value>(&peer_msg.msg) {
                                    ...
                                    if inner.get("type").and_then(|t| t.as_str()) == Some("offer") {
                                        if let Some(sdp) = inner.get("sdp").and_then(|s| s.as_str()) {
                                            ...
                                            let _ = event_tx.send(SignalingEvent::SdpOffer(sdp.to_string())).await;
                                        }
                                    }
                                    // ICE Candidate
                                    else if inner.get("candidate").is_some() {
                                        if let Ok(candidate) = serde_json::from_value::<IceCandidate>(inner) {
                                            info!("Received ICE candidate: {}", &candidate.candidate[..candidate.candidate.len().min(60)]);
                                            let _ = event_tx.send(SignalingEvent::IceCandidate(candidate)).await;
                                        }
                                    }
                                }
                            }
