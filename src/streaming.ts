...
/**
 * Poll for messages from native signaling connection
 */
async function startNativeSignalingPoll(
  sessionId: string,
  config: WebRtcConfig
): Promise<void> {
  console.log("Starting native signaling poll for session:", sessionId);
  ...
  const poll = async () => {
    ...
    try {
      ...
      const connected = await invoke<boolean>("is_signaling_connected", {
        sessionId: sessionId,
      });

      if (!connected && pollCount > 5) {
        console.log("Native signaling disconnected");
        return;
      }

      // Get new messages
      const messages = await invoke<string[]>("get_signaling_messages", {
        sessionId: sessionId,
      });

      for (const msg of messages) {
        console.log("Native signaling message:", msg.substring(0, 200));
        await processNativeSignalingMessage(msg, config);
      }
      ...
      console.error("Native signaling poll error:", e);
    }
  };
...
/**
 * Process message from native signaling
 */
async function processNativeSignalingMessage(
  message: string,
  config: WebRtcConfig
): Promise<void> {
  // Check if it's an RTSP response
  if (message.startsWith("RTSP/1.0")) {
    console.log("RTSP Response received");

    // Parse RTSP response
    if (message.includes("200 OK")) {
      console.log("RTSP OK - signaling successful");

      // Look for SDP in the response
      const sdpMatch = message.match(/\r\n\r\n([\s\S]+)/);
      if (sdpMatch && sdpMatch[1]) {
        const sdp = sdpMatch[1].trim();
        if (sdp.startsWith("v=0") || sdp.includes("m=video") || sdp.includes("m=audio")) {
          console.log("SDP found in RTSP response, length:", sdp.length);
          // TODO: Set up WebRTC with this SDP
        }
      }
    }
  }
  ...
  try {
    ...
    if (json.type === "offer" && json.sdp) {
      console.log("JSON offer received");
      // TODO: Handle SDP offer
    }
  } catch {
    // Not JSON
  }
